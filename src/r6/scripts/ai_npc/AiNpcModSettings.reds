...
public class AiNpcSettingsService extends ScriptableService {
    ...
    @runtimeProperty("ModSettings.description", "Which AI service generates the replies. OpenRouter is the supported way to play. The two CLI lanes are for mod authors testing their own work: playing through a coding-agent subscription is outside what those plans are sold for, and the provider may limit or suspend your account without warning.")
